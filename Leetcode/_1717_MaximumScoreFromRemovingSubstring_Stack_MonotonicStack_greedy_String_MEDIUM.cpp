...
public:
    int maximumGain(string s, int x, int y) {
        ...
        totalScore += removedPairsCount * max(x, y);        //max because high priority removed
        ...
        totalScore += removedPairsCount * min(x, y);        //min because low priority removed
...
int main(){
    ...
    string s = "cbaab";
    int x = 1, y = 2;
    cout << sol.maximumGain(s, x, y) << endl; // Output: 3
    return 0;
}